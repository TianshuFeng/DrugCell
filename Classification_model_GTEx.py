...
class Drugcell_Vae(nn.Module):
    ...
    def __init__(self, term_size_map, term_direct_gene_map, dG, ngene, root, 
        ...
        self.final_classifier = nn.Sequential(
            nn.Linear(self.num_hiddens_final * 2, 512),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.3),
...
#            print("term\t%s\tterm_size\t%d\tnum_hiddens\t%d" % (term, term_size, num_output))
    ...
    def forward(self, x):
        ...
        final_emb = term_NN_out_map['final']

        logits = self.final_classifier(final_emb)  

        return logits, term_NN_out_map, term_original_children, aux_out_map, aux_cancer_map


        
    
    def loss_log_vae(self, logits, y):

        class_loss = F.cross_entropy(logits, y, reduction='mean')
        log_loss = class_loss  

        KLD = torch.tensor(0.0, device=logits.device) 

        return log_loss, class_loss, KLD
